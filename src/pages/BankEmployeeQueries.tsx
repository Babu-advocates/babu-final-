...
import { HelpCircle, Send, User, FileText, Paperclip, Upload, X, MessageSquare, Download, CheckCheck } from "lucide-react";
...
function BankEmployeeQueries() {
  ...
  const openQueryModal = (applicationId: string) => {
    ...
    // Mark all incoming messages for this application as read when opening
    markMessagesAsRead(applicationId);
  };
  ...
  useEffect(() => {
    ...
    const group = applicationGroups[selectedApplicationId];
    if (!group) return;
    const hasUnreadForMe = group.queries.some(
      (q) => !q.is_read && q.sender_type !== currentUser.type
    );
    if (hasUnreadForMe) {
      markMessagesAsRead(selectedApplicationId);
    }
  }, [selectedApplicationId, queries]);
                              ...
                              >
                                ...
                                {query.sender_type === currentUser.type && (
                                  <div className="mt-1 flex justify-end items-center gap-1">
                                    <CheckCheck className={`h-4 w-4 transition-colors ${query.is_read ? 'text-blue-400' : 'text-gray-300'}`} />
                                  </div>
                                )}
                              </div>
                            </div>
