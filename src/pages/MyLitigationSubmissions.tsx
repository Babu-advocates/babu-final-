  ...
  if (isEditing) {
    return (
      <TableRow className="bg-amber-50">
        <TableCell className="font-medium text-muted-foreground">{selectedCase?.case_no || '-'}</TableCell>
        <TableCell className="text-muted-foreground">{selectedCase?.case_type || '-'}</TableCell>
        <TableCell className="text-muted-foreground">{selectedCase?.court_name || '-'}</TableCell>
        <TableCell>
          <Popover>
...
export default function MyLitigationSubmissions() {
  ...
  const [casesNeedingUpdate, setCasesNeedingUpdate] = useState<Set<string>>(new Set());
  ...
  // Re-evaluate red highlight whenever cases change (ensures next_hearing_date is considered)
  useEffect(() => {
    fetchCasesNeedingUpdate();
  }, [myCases]);
  ...
  const fetchAllCaseNextHearingDates = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('litigation_case_id, hearing_date')
  ...
  const fetchCasesNeedingUpdate = async () => {
    try {
      ...
      // Build a quick lookup for case -> status from current state
      const caseStatusMap = new Map<string, string>();
      myCases.forEach((c: any) => caseStatusMap.set(c.id, (c.status || '').toLowerCase()));
      
      // Get all history entries with hearing dates
      const { data, error } = await supabase
        .from('litigation_case_history')
        .select('litigation_case_id, hearing_date')
      ...
      if (error) {
        console.error('Error fetching case history for updates:', error);
        return;
      }

      const needsUpdateSet = new Set<string>();
      
      // Group entries by case and find the latest hearing date for each case
      const caseLatestHearingMap = new Map<string, string>();
      
      data?.forEach((entry: any) => {
        const hearingDate = formatInTimeZone(new Date(entry.hearing_date), 'Asia/Kolkata', 'yyyy-MM-dd');
        const caseId = entry.litigation_case_id;
        
        // Keep track of the latest (most recent) hearing date for each case
        const currentLatest = caseLatestHearingMap.get(caseId);
        if (!currentLatest || hearingDate > currentLatest) {
          caseLatestHearingMap.set(caseId, hearingDate);
        }
      });

      // Highlight when latest hearing date is today or earlier (overdue),
      // excluding cases with status Decreed or Dismissed
      caseLatestHearingMap.forEach((latestHearing, caseId) => {
        const status = caseStatusMap.get(caseId) || '';
        const isClosed = status === 'decreed' || status === 'dismissed';
        if (!isClosed && latestHearing <= todayIST) {
          needsUpdateSet.add(caseId);
        }
      });

      setCasesNeedingUpdate(needsUpdateSet);
    } catch (error) {
      console.error('Error fetching cases needing update:', error);
    }
  };
  ...
  const filteredCases = myCases.filter((caseItem) => {
    ...
    const matchesBank = !selectedBank || 
      (caseItem.bank_name && caseItem.bank_name.toLowerCase().includes(selectedBank.toLowerCase()));

    const matchesCaseType = !caseTypeFilter || 
      (caseItem.case_type && caseItem.case_type.toLowerCase().includes(caseTypeFilter.toLowerCase()));

    const matchesCourt = !courtFilter || 
      (caseItem.court_name && caseItem.court_name.toLowerCase().includes(courtFilter.toLowerCase()));

    const matchesDistrict = !districtFilter || 
      (caseItem.court_district && caseItem.court_district.toLowerCase().includes(districtFilter.toLowerCase()));
    ...
    if (nextHearingStartDate || nextHearingEndDate) {
      const nextHearingDateStr = casesWithNextHearing.get(caseItem.id);
      if (nextHearingDateStr) {
        ...
        // If no hearing date found in history, exclude from filter
        matchesNextHearingDateRange = false;
      }
    }

    return matchesSearch && matchesStatus && matchesBank && matchesCaseType && 
           matchesCourt && matchesDistrict && matchesNextHearingDateRange;
  });
  ...
  const handleViewCaseHistory = async (caseItem: any) => {
    ...
    // Fetch history entries from database first
    try {
      const { data, error } = await supabase
        .from('litigation_case_history')
        .select('*')
      ...
      if (error) {
        ...
        setHistoryEntries([]);
        // Initialize with filing date if no history
        setNewHistoryEntry({
  ...
  const handleAddHistoryEntry = async () => {
    if (!newHistoryEntry.business_on_date || !newHistoryEntry.hearing_date || 
    ...
    try {
      ...
      if (error) {
        ...
        // Immediately refresh highlight state and next hearing cache
        fetchAllCaseNextHearingDates();
        fetchCasesNeedingUpdate();
      }
    } catch (error) {
              ...
              <CardContent className="p-6">
                ...
                <div className="mb-6 space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-sm font-medium min-w-[150px]">Next Hearing Date:</span>
                    <div className="flex gap-2 items-center flex-1">
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className={cn("justify-start text-left font-normal flex-1", !nextHearingStartDate && "text-muted-foreground")}>
                            <Calendar className="mr-2 h-4 w-4" />
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className={cn("justify-start text-left font-normal flex-1", !nextHearingEndDate && "text-muted-foreground")}>
                            <Calendar className="mr-2 h-4 w-4" />
                      ...
                      {(nextHearingStartDate || nextHearingEndDate) && 
                  ...
                  {/* Clear All Filters */}
                  {(searchQuery || selectedBank || filterStatus !== "all" || caseTypeFilter || courtFilter || districtFilter || nextHearingStartDate || nextHearingEndDate) && 
                      ...
                      <TableBody>
                        {filteredCases.length === 0 ? (
                          ...
                          filteredCases.map((caseItem) => {
                            ...
                            // Check if this case needs update (hearing today or past but next hearing not set)
                            const needsUpdate = casesNeedingUpdate.has(caseItem.id);
                            
                            return (
                              <TableRow 
                                key={caseItem.id}
                                className={cn(needsUpdate && "bg-red-600 text-white hover:bg-red-700")}
                              >
                                <TableCell className={cn("font-medium", needsUpdate && "text-white")}>{caseItem.case_no}</TableCell>
                                <TableCell>
                                  <Select 
                                    value={caseItem.status} 
                                    onValueChange={(value) => handleStatusChange(caseItem.id, value)}
                                  >
                                    <SelectTrigger className={cn("w-[140px]", needsUpdate && "bg-transparent text-white border-white")}>
                                      <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                      <SelectItem value="Pending">Pending</SelectItem>
                                ...
                                <TableCell className={cn(needsUpdate && "text-white")}>{name}</TableCell>
                                <TableCell className={cn(needsUpdate && "text-white")}>{caseItem.bank_name || '-'}</TableCell>
                                <TableCell className={cn(needsUpdate && "text-white")}>{caseItem.court_name}</TableCell>
                                <TableCell className={cn(needsUpdate && "text-white")}>{caseItem.case_type}</TableCell>
                                <TableCell className={cn(needsUpdate && "text-white")}>{format(new Date(caseItem.filing_date), 'dd MMM yyyy')}</TableCell>
                                 <TableCell className="text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    <Button
                                      variant="ghost"
                                      ...
                                      title="View details"
                                      className={cn(needsUpdate && "text-white hover:bg-red-500 hover:text-white")}
                                    >
                                    ...
                                    <Button
                                      ...
                                      className={cn(
                                        hasEditAccess(caseItem.id) ? "text-blue-600" : "",
                                        needsUpdate && "text-white hover:bg-red-500 hover:text-white"
                                      )}
                                    >
                                    ...
                                    {getEditRequestStatus(caseItem.id)?.status === 'pending' && (
                                      <Badge variant="outline" className={cn("text-xs", needsUpdate && "border-white text-white")}>Pending</Badge>
                                    )}
              ...
              <div className="rounded-md border">
                <Table>
                  <TableHeader className="bg-blue-900">
                    <TableRow>
                      <TableHead className="text-white font-semibold">Case No</TableHead>
                      <TableHead className="text-white font-semibold">Case Type</TableHead>
                      <TableHead className="text-white font-semibold">Court Name</TableHead>
                      <TableHead className="text-white font-semibold">Hearing Date</TableHead>
                      <TableHead className="text-white font-semibold">Next Hearing Date</TableHead>
