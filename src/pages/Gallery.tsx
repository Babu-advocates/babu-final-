      ...
      try {
        ...
        if (data) {
          const images = data.map((img) => ({
            src: img.file_path,
            title: img.title || img.file_name.replace(/\.[^/.]+$/, "").replace(/[-_]/g, ' '),
          ...
          {loading ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
              {galleryImages.map((image, index) => (
                <div 
                  key={index}
                  className="group relative overflow-hidden rounded-xl shadow-elegant hover:shadow-glow transition-all duration-300 h-72"
                >
                  <CachedImage
                    src={image.src}
                    alt={image.title}
                    className="w-full h-72 object-cover group-hover:scale-110 transition-all duration-500 cursor-pointer"
