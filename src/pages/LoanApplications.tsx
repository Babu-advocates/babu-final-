...
import { Search, Filter, Building2, FileText, User, Calendar, CreditCard, Phone, Mail, MapPin, Download, Upload, Check, Trash2, ArrowUpRight, Loader2, FileDown, Edit } from "lucide-react";
...
import { getStatusColor, getDisplayStatus, filterApplications } from "@/lib/applicationFilters";
import { addDigitalSignatureToPDF } from "@/lib/pdfSignature";
const LoanApplications = () => {
  ...
  const { loanApplications, setLoanApplications, loading, fetchApplications } = useApplications(isAdminRoute);
  const { employees, loadingEmployees, fetchEmployees } = useEmployees();
  ...
  const filteredApplications = filterApplications(loanApplications, {
    searchTerm,
    selectedBank,
    selectedBankBranch,
    statusFilter,
  ...
  const exportToExcel = () => {
    try {
      // Prepare data for export with all fields from view details
      const exportData = filteredApplications.map(app => ({
        'Application ID': app.applicationNumber,
        'Bank Application Number': app.bank_application_no || 'N/A',
        'Status': getDisplayStatus(app, isAdminRoute),
        'Application Type': app.applicationType,
        'Bank Name': app.bankName,
        'Branch Name': app.branch_name || 'N/A',
        'Submission Date': format(new Date(app.submission_date || app.date), 'dd-MM-yyyy'),
        'Sanction Date': app.sanction_date ? format(new Date(app.sanction_date), 'dd-MM-yyyy') : 'N/A',
        'Assigned To': app.assigned_to_username || 'Not Assigned',
        
        // Applicant Details
        'Borrower Name': app.borrower_name || app.name,
        'Customer ID': app.customer_id || 'N/A',
        ...
        // Property Details
        'Nature of Property': app.nature_of_property || 'N/A',
        'Location of Property': app.location_of_property || 'N/A',
        'Survey Number': app.survey_number || 'N/A',
        'Extent of Property': app.extent_of_property || 'N/A',
        'Plot Number': app.plot_no || 'N/A',
        'Layout Name': app.layout_name || 'N/A',
        'Owner Name': app.owner_name || 'N/A',
        
        // Salesman Details
        'Salesman Name': app.salesman_name || 'N/A',
        'Salesman Contact': app.salesman_contact || 'N/A',
        'Salesman Email': app.salesman_email || 'N/A',
        'Office Branch': app.office_branch || 'N/A',
      }));
      ...
      const columnWidths = [
        ...
        { wch: 20 }, // Bank Application Number
        { wch: 15 }, // Status
        { wch: 18 }, // Application Type
        { wch: 25 }, // Bank Name
        { wch: 20 }, // Branch Name
        { wch: 15 }, // Submission Date
        { wch: 15 }, // Sanction Date
        { wch: 20 }, // Assigned To
        ...
        { wch: 20 }, // Nature of Property
        { wch: 25 }, // Location of Property
        { wch: 15 }, // Survey Number
        { wch: 18 }, // Extent of Property
        { wch: 15 }, // Plot Number
        { wch: 20 }, // Layout Name
        { wch: 20 }, // Owner Name
        { wch: 20 }, // Salesman Name
        { wch: 15 }, // Salesman Contact
        { wch: 25 }, // Salesman Email
        { wch: 20 }, // Office Branch
      ];
            ...
            <Table className="w-full">
              ...
              <TableBody>
                {filteredApplications.map((application, index) => <TableRow key={application.id} className="hover:bg-blue-50/50 transition-colors duration-200 border-b border-slate-100">
                    ...
                    <TableCell>
                      <Badge className={`${getStatusColor(getDisplayStatus(application, isAdminRoute))} font-medium px-3 py-1`}>
                        {getDisplayStatus(application, isAdminRoute)}
                      </Badge>
                    </TableCell>
