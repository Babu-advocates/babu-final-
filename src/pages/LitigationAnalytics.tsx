...
import { Scale, Clock, Eye, CheckCircle, AlertTriangle, LogOut, Filter, Calendar, BarChart3 } from "lucide-react";
...
export default function LitigationAnalytics() {
  ...
  const calculateStats = (litigationCases: any[]) => {
    const total = litigationCases.length;
    const pending = litigationCases.filter(c => c.status === 'pending').length;
    const active = litigationCases.filter(c => c.status === 'active' || c.status === 'ongoing').length;
    const completed = litigationCases.filter(c => c.status === 'completed' || c.status === 'closed').length;
    
    // Delayed: pending for more than 7 days
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const delayed = litigationCases.filter(c => {
      const createdAt = new Date(c.created_at);
      return (c.status === 'pending' || c.status === 'active') && createdAt < sevenDaysAgo;
    }).length;
  ...
  const calculateChartData = (litigationCases: any[]) => {
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    
    const data = months.map((month, index) => {
      const monthCases = litigationCases.filter(c => {
      ...
      return {
        ...
        Active: monthCases.filter(c => c.status === 'active' || c.status === 'ongoing').length,
        Completed: monthCases.filter(c => c.status === 'completed' || c.status === 'closed').length,
      };
    });
