...
const LoanRecovery = () => {
  ...
  const [nextHearingStartDate, setNextHearingStartDate] = useState<Date | undefined>(undefined);
  const [nextHearingEndDate, setNextHearingEndDate] = useState<Date | undefined>(undefined);
  const [litigationCases, setLitigationCases] = useState<any[]>([]);
  const [casesWithNextHearing, setCasesWithNextHearing] = useState<Map<string, string>>(new Map());
  ...
  useEffect(() => {
    ...
    fetchAllCaseNextHearingDates();

    // Set up real-time subscription
    const channel = supabase.channel('litigation-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  const fetchAllCaseNextHearingDates = async () => {
    try {
      // Fetch all case histories and get the latest hearing date for each case
      const {
        data,
        error
      } = await supabase.from('litigation_case_history').select('litigation_case_id, hearing_date').order('hearing_date', {
        ascending: false
      });
      if (error) {
        console.error('Error fetching case history dates:', error);
        return;
      }

      // Create a map of case_id -> latest next hearing date
      const nextHearingMap = new Map<string, string>();
      data?.forEach((entry: any) => {
        if (!nextHearingMap.has(entry.litigation_case_id)) {
          nextHearingMap.set(entry.litigation_case_id, entry.hearing_date);
        }
      });
      setCasesWithNextHearing(nextHearingMap);
    } catch (error) {
      console.error('Error fetching case history dates:', error);
    }
  };
  ...
  const fetchLitigationAccessAccounts = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('litigation_access_accounts').select('username').eq('is_active', true);
      if (error) {
  ...
  const banks = Array.from(new Set(litigationCases.filter(c => c.bank_name).map(c => c.bank_name)));
  const statuses = Array.from(new Set(litigationCases.filter(c => c.status).map(c => c.status)));
  const caseTypes = Array.from(new Set(litigationCases.filter(c => c.case_type).map(c => c.case_type)));
  const courts = Array.from(new Set(litigationCases.filter(c => c.court_name).map(c => c.court_name)));
  const districts = Array.from(new Set(litigationCases.filter(c => c.court_district).map(c => c.court_district)));
  const categories = Array.from(new Set(litigationCases.filter(c => c.category).map(c => c.category)));
  ...
  const handleStatusChange = async (caseId: string, newStatus: string) => {
    try {
      const {
        error
      } = await supabase.from('litigation_cases').update({
        status: newStatus
      }).eq('id', caseId);
      if (error) {
        ...
        // Update local state
        setLitigationCases(litigationCases.map(c => c.id === caseId ? {
          ...c,
          status: newStatus
        } : c));
      }
  ...
  const filteredApplications = litigationCases.filter(litigationCase => {
    const name = litigationCase.category === 'bank' ? litigationCase.borrower_name : litigationCase.petitioner_name;
    const bank = litigationCase.bank_name || '';
    const matchesSearch = name?.toLowerCase().includes(searchTerm.toLowerCase()) || litigationCase.case_no?.toLowerCase().includes(searchTerm.toLowerCase()) || bank.toLowerCase().includes(searchTerm.toLowerCase()) || litigationCase.case_type?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesBank = !selectedBank || litigationCase.bank_name?.toLowerCase().includes(selectedBank.toLowerCase());

    // Status filter
    const matchesStatus = !statusFilter || litigationCase.status?.toLowerCase().includes(statusFilter.toLowerCase());

    // Case type filter
    const matchesCaseType = !caseTypeFilter || litigationCase.case_type?.toLowerCase().includes(caseTypeFilter.toLowerCase());

    // Court filter
    const matchesCourt = !courtFilter || litigationCase.court_name?.toLowerCase().includes(courtFilter.toLowerCase());

    // District filter
    const matchesDistrict = !districtFilter || litigationCase.court_district?.toLowerCase().includes(districtFilter.toLowerCase());

    // Category filter (bank/private)
    const matchesCategory = !categoryFilter || litigationCase.category?.toLowerCase().includes(categoryFilter.toLowerCase());
    ...
    // Next hearing date range filtering based on case history
    let matchesNextHearingDateRange = true;
    if (nextHearingStartDate || nextHearingEndDate) {
      const nextHearingDateStr = casesWithNextHearing.get(litigationCase.id);
      if (nextHearingDateStr) {
        const nextHearingDate = new Date(nextHearingDateStr);
        if (nextHearingStartDate && nextHearingEndDate) {
          const start = new Date(nextHearingStartDate);
          start.setHours(0, 0, 0, 0);
          const end = new Date(nextHearingEndDate);
          end.setHours(23, 59, 59, 999);
          matchesNextHearingDateRange = nextHearingDate >= start && nextHearingDate <= end;
        } else if (nextHearingStartDate) {
          const start = new Date(nextHearingStartDate);
          start.setHours(0, 0, 0, 0);
          matchesNextHearingDateRange = nextHearingDate >= start;
        } else if (nextHearingEndDate) {
          const end = new Date(nextHearingEndDate);
          end.setHours(23, 59, 59, 999);
          matchesNextHearingDateRange = nextHearingDate <= end;
        }
      } else {
        // If no next hearing date exists in history, don't include it in filtered results
        matchesNextHearingDateRange = false;
      }
    }
    return matchesSearch && matchesBank && matchesStatus && matchesCaseType && matchesCourt && matchesDistrict && matchesCategory && matchesDateRange && matchesNextHearingDateRange;
  });
  ...
  const handleViewDetails = async (litigationCase: any) => {
    ...
    try {
      const {
        data,
        error
      } = await (supabase as any).from('litigation_case_visibility').select('litigation_access_username').eq('litigation_case_id', litigationCase.id);
      if (error) {
  ...
  const handleViewHistory = async (litigationCase: any) => {
    ...
    // Fetch case history
    const history = await fetchCaseHistory(litigationCase.id);

    // Initialize new history entry with the latest hearing_date as business_on_date
  ...
  const fetchCaseHistory = async (caseId: string) => {
    try {
      const {
        data,
        error
      } = await supabase.from('litigation_case_history').select('*').eq('litigation_case_id', caseId).order('created_at', {
        ascending: true
      });
      if (error) {
  ...
  const handleAddHistoryEntry = async () => {
    ...
    if (!newHistoryEntry.hearing_date) {
      showToast.error('Please fill in the next hearing date');
      return;
    }
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('litigation_case_history').insert({
        litigation_case_id: selectedCase.id,
        registration_number: selectedCase.case_no,
        judge_name: selectedCase.case_type,
      ...
      if (error) {
        ...
        // Reset form: Set business_on_date to the just-added hearing_date, clear hearing_date for next entry
        setNewHistoryEntry({
  ...
  const handleEditHistoryEntry = async (entry: any) => {
    if (editingHistoryId === entry.id) {
      ...
      try {
        const {
          error
        } = await supabase.from('litigation_case_history').update({
  ...
  const handleDeleteHistoryEntry = async (entryId: string) => {
    try {
      const {
        error
      } = await supabase.from('litigation_case_history').delete().eq('id', entryId);
      if (error) {
  ...
  const updateHistoryEntry = (entryId: string, field: string, value: string) => {
    setHistoryEntries(historyEntries.map(entry => entry.id === entryId ? {
      ...entry,
      [field]: value
    } : entry));
  };
  const toggleAccountVisibility = async (username: string) => {
    ...
    try {
      if (isCurrentlyVisible) {
        ...
        const {
          error
        } = await (supabase as any).from('litigation_case_visibility').delete().eq('litigation_case_id', selectedCase.id).eq('litigation_access_username', username);
        if (error) {
        ...
        const {
          error
        } = await (supabase as any).from('litigation_case_visibility').insert({
          litigation_case_id: selectedCase.id,
  ...
  const handleNoVisible = async () => {
    ...
    try {
      ...
      const {
        error
      } = await (supabase as any).from('litigation_case_visibility').delete().eq('litigation_case_id', selectedCase.id);
      if (error) {
  ...
  return <SidebarProvider>
          ...
          <header className="bg-gradient-to-r from-white/95 to-blue-50/95 backdrop-blur-sm shadow-elegant border-b border-white/20">
            <div className="px-6">
              <div className="flex justify-between items-center h-16">
                ...
                <Button onClick={handleExportToExcel} variant="outline" className="border-green-600 text-green-600 hover:bg-green-50">
            ...
            <div className="px-6 py-8">
              ...
              <div className="mb-8 bg-white/80 backdrop-blur-sm rounded-lg shadow-card border border-white/20 p-6">
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <Input placeholder="Search District..." value={districtFilter} onChange={e => setDistrictFilter(e.target.value)} className="bg-white/50 border-slate-200 focus:border-blue-300 focus:ring-blue-200" />
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-sm font-medium text-slate-700 min-w-[150px]">Next Hearing Date:</span>
                      <div className="flex gap-2 items-center flex-1">
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="outline" className={cn("justify-start text-left font-normal bg-white/50 border-slate-200 flex-1", !nextHearingStartDate && "text-muted-foreground")}>
                              <CalendarIcon className="mr-2 h-4 w-4" />
                              {nextHearingStartDate ? format(nextHearingStartDate, "dd-MM-yyyy") : "Start Date"}
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0" align="start">
                            <CalendarComponent mode="single" selected={nextHearingStartDate} onSelect={setNextHearingStartDate} initialFocus className="pointer-events-auto" />
                          </PopoverContent>
                        </Popover>

                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="outline" className={cn("justify-start text-left font-normal bg-white/50 border-slate-200 flex-1", !nextHearingEndDate && "text-muted-foreground")}>
                              <CalendarIcon className="mr-2 h-4 w-4" />
                              {nextHearingEndDate ? format(nextHearingEndDate, "dd-MM-yyyy") : "End Date"}
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0" align="start">
                            <CalendarComponent mode="single" selected={nextHearingEndDate} onSelect={setNextHearingEndDate} initialFocus className="pointer-events-auto" />
                          </PopoverContent>
                        </Popover>

                        {(nextHearingStartDate || nextHearingEndDate) && <Button variant="ghost" size="sm" onClick={() => {
                        setNextHearingStartDate(undefined);
                        setNextHearingEndDate(undefined);
                      }} className="shrink-0">
                            <X className="h-4 w-4" />
                          </Button>}
                      </div>
                  ...
                  {(searchTerm || selectedBank || statusFilter || caseTypeFilter || courtFilter || districtFilter || categoryFilter || startDate || endDate || nextHearingStartDate || nextHearingEndDate) && <div className="flex justify-end">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-card border border-white/20 overflow-hidden">
                {loading ? <div className="text-center py-12">
                    ...
                    const name = litigationCase.category === 'bank' ? litigationCase.borrower_name : litigationCase.petitioner_name;
                    return <TableRow key={litigationCase.id} className="hover:bg-green-50/50 transition-colors duration-200">
                            ...
                            <TableCell>
                              <Select value={litigationCase.status} onValueChange={value => handleStatusChange(litigationCase.id, value)}>
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleEdit(litigationCase)} className="border-blue-600 text-blue-600 hover:bg-blue-50">
                                  <Edit2 className="h-4 w-4 mr-1" />
                                  Edit
                                </Button>
                                <Button className="bg-[#334155] hover:bg-[#475569] text-white" size="sm" onClick={() => handleViewDetails(litigationCase)}>
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-4">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-56 bg-white z-50">
                  ...
                  {litigationAccessAccounts.map(account => <DropdownMenuCheckboxItem key={account.username} checked={visibleToAccounts.includes(account.username)} onCheckedChange={() => toggleAccountVisibility(account.username)} className="cursor-pointer">
                      {account.username}
          ...
          {selectedCase && <div className="space-y-6 mt-4">
              {/* Case Information */}
              <div className="bg-slate-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {selectedCase.next_hearing_date && <div>
              ...
              {selectedCase.category === 'bank' && <div className="bg-blue-50 p-4 rounded-lg">
                  <h3 className="font-semibold text-lg mb-3 flex items-center">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {selectedCase.co_borrower_name && <div>
              ...
              {selectedCase.category === 'private' && <div className="bg-purple-50 p-4 rounded-lg">
                  <h3 className="font-semibold text-lg mb-3 flex items-center">
              ...
              <div className="bg-amber-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {selectedCase.total_advocate_fees && <div>
                  ...
                  {selectedCase.judgement_date && <div>
              ...
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-lg">
                ...
                <div className="bg-white p-6 rounded-lg mb-6 border border-indigo-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <FloatingInput label="Case No" value={selectedCase?.case_no || ""} disabled className="bg-gray-50" />
                    <FloatingInput label="Case Type" value={selectedCase?.case_type || ""} disabled className="bg-gray-50" />
                    <FloatingInput label="Court Name" value={selectedCase?.court_name || ""} disabled className="bg-gray-50" />
                    <DatePicker label="Hearing Date" value={newHistoryEntry.business_on_date} onChange={date => setNewHistoryEntry({
                  ...newHistoryEntry,
                  business_on_date: date
                })} showLabelOutside={false} />
                    <DatePicker label="Next Hearing Date" value={newHistoryEntry.hearing_date} onChange={date => setNewHistoryEntry({
                  ...newHistoryEntry,
                  hearing_date: date
                })} showLabelOutside={false} />
                    <div className="md:col-span-2">
                      <FloatingInput label="Purpose of Hearing" value={newHistoryEntry.purpose_of_hearing} onChange={e => setNewHistoryEntry({
                    ...newHistoryEntry,
                    purpose_of_hearing: e.target.value
                  })} />
                    </div>
                  </div>
                  <Button onClick={handleAddHistoryEntry} className="mt-5 bg-indigo-600 hover:bg-indigo-700" disabled={isSubmittingHistory}>
                    {isSubmittingHistory ? <>
                ...
                {historyEntries.length > 0 ? <div className="bg-white rounded-lg border border-indigo-200 overflow-hidden">
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-indigo-50">
                          <TableHead className="text-sm font-semibold">Case No</TableHead>
                          <TableHead className="text-sm font-semibold">Case Type</TableHead>
                          <TableHead className="text-sm font-semibold">Court Name</TableHead>
                          <TableHead className="text-sm font-semibold">Hearing Date</TableHead>
                          <TableHead className="text-sm font-semibold">Next Hearing Date</TableHead>
                      ...
                      <TableBody>
                        {historyEntries.map(entry => <TableRow key={entry.id}>
                            ...
                            <TableCell className="text-sm">
                              {editingHistoryId === entry.id ? <DatePicker label="Hearing Date" value={entry.business_on_date || ""} onChange={date => updateHistoryEntry(entry.id, 'business_on_date', date)} showLabelOutside={false} /> : entry.business_on_date ? format(new Date(entry.business_on_date), 'dd MMM yyyy') : "-"}
                            </TableCell>
                            <TableCell className="text-sm">
                              {editingHistoryId === entry.id ? <DatePicker label="Next Hearing Date" value={entry.hearing_date || ""} onChange={date => updateHistoryEntry(entry.id, 'hearing_date', date)} showLabelOutside={false} /> : entry.hearing_date ? format(new Date(entry.hearing_date), 'dd MMM yyyy') : "-"}
                            </TableCell>
                            <TableCell className="text-sm">
                              {editingHistoryId === entry.id ? <Input value={entry.purpose_of_hearing || ""} onChange={e => updateHistoryEntry(entry.id, 'purpose_of_hearing', e.target.value)} className="text-sm h-10" /> : entry.purpose_of_hearing || "-"}
                            </TableCell>
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-2">
                                {editingHistoryId === entry.id ? <>
                                    <Button size="sm" variant="ghost" onClick={() => handleEditHistoryEntry(entry)} className="h-8 w-8 p-0" disabled={isUpdatingHistory}>
                                      {isUpdatingHistory ? <Loader2 className="h-4 w-4 text-green-600 animate-spin" /> : <Save className="h-4 w-4 text-green-600" />}
                                    </Button>
                                    <Button size="sm" variant="ghost" onClick={() => setEditingHistoryId(null)} className="h-8 w-8 p-0">
                                      <X className="h-4 w-4 text-slate-600" />
                                    </Button>
                                  </> : <>
                                    <Button size="sm" variant="ghost" onClick={() => setEditingHistoryId(entry.id)} className="h-8 w-8 p-0">
                                      <Edit2 className="h-4 w-4 text-blue-600" />
                                    </Button>
                                    <Button size="sm" variant="ghost" onClick={() => setDeletingEntryId(entry.id)} className="h-8 w-8 p-0">
                  ...
                  </div> : <div className="bg-white rounded-lg border border-indigo-200 p-8 text-center">
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {editFormData && <div className="space-y-6 mt-4">
              {/* Basic Information */}
              <div className="bg-slate-50 p-4 rounded-lg">
                <h3 className="font-semibold text-lg mb-4">Basic Information</h3>
                <div className="grid grid-cols-2 gap-4">
                  <FloatingInput label="Case Number" value={editFormData.case_no || ""} onChange={e => setEditFormData({
                ...editFormData,
                ...
                category: value
                  ...
                  <FloatingInput label="Case Type" value={editFormData.case_type || ""} onChange={e => setEditFormData({
                ...editFormData,
                case_type: e.target.value
              })} />
                  <Select value={editFormData.status || ""} onValueChange={value => setEditFormData({
                ...editFormData,
                status: value
              })}>
                    <SelectTrigger>
                  ...
                  <FloatingInput label="Court Name" value={editFormData.court_name || ""} onChange={e => setEditFormData({
                ...editFormData,
                court_name: e.target.value
              })} />
                  <FloatingInput label="Court District" value={editFormData.court_district || ""} onChange={e => setEditFormData({
                ...editFormData,
                court_district: e.target.value
              })} />
                  <DatePicker label="Filing Date" value={editFormData.filing_date || ""} onChange={date => setEditFormData({
                ...editFormData,
                filing_date: date
              })} showLabelOutside={false} />
                  <DatePicker label="Next Hearing Date" value={editFormData.next_hearing_date || ""} onChange={date => setEditFormData({
                ...editFormData,
              ...
              {editFormData.category === 'bank' && <div className="bg-blue-50 p-4 rounded-lg">
                  <h3 className="font-semibold text-lg mb-4">Bank Details</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <FloatingInput label="Bank Name" value={editFormData.bank_name || ""} onChange={e => setEditFormData({
                ...editFormData,
                bank_name: e.target.value
              })} />
                    <FloatingInput label="Branch Name" value={editFormData.branch_name || ""} onChange={e => setEditFormData({
                ...editFormData,
                branch_name: e.target.value
              })} />
                    <FloatingInput label="Account Number" value={editFormData.account_no || ""} onChange={e => setEditFormData({
                ...editFormData,
                account_no: e.target.value
              })} />
                    <FloatingInput label="Loan Amount" type="number" value={editFormData.loan_amount || ""} onChange={e => setEditFormData({
                ...editFormData,
                loan_amount: parseFloat(e.target.value) || null
              })} />
                    <FloatingInput label="Borrower Name" value={editFormData.borrower_name || ""} onChange={e => setEditFormData({
                ...editFormData,
                borrower_name: e.target.value
              })} />
                    <FloatingInput label="Co-Borrower Name" value={editFormData.co_borrower_name || ""} onChange={e => setEditFormData({
                ...editFormData,
                co_borrower_name: e.target.value
              })} />
                  </div>
                </div>}

              {/* Party Details - shown only for private category */}
              {editFormData.category === 'private' && <div className="bg-purple-50 p-4 rounded-lg">
                  <h3 className="font-semibold text-lg mb-4">Party Details</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <FloatingInput label="Petitioner Name" value={editFormData.petitioner_name || ""} onChange={e => setEditFormData({
                ...editFormData,
                petitioner_name: e.target.value
              })} />
                    <FloatingInput label="Respondent Name" value={editFormData.respondent_name || ""} onChange={e => setEditFormData({
                ...editFormData,
                respondent_name: e.target.value
              })} />
                    <div className="col-span-2">
                      <FloatingInput label="Petitioner Address" value={editFormData.petitioner_address || ""} onChange={e => setEditFormData({
                  ...editFormData,
                  petitioner_address: e.target.value
                })} />
                    </div>
                    <div className="col-span-2">
                      <FloatingInput label="Respondent Address" value={editFormData.respondent_address || ""} onChange={e => setEditFormData({
                  ...editFormData,
              ...
              <div className="bg-amber-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FloatingInput label="Total Advocate Fees" type="number" value={editFormData.total_advocate_fees || ""} onChange={e => setEditFormData({
                ...editFormData,
                total_advocate_fees: parseFloat(e.target.value) || null
              })} />
                  <FloatingInput label="Initial Fees" type="number" value={editFormData.initial_fees || ""} onChange={e => setEditFormData({
                ...editFormData,
                initial_fees: parseFloat(e.target.value) || null
              })} />
                  <DatePicker label="Initial Fees Received On" value={editFormData.initial_fees_received_on || ""} onChange={date => setEditFormData({
                ...editFormData,
                initial_fees_received_on: date
              })} showLabelOutside={false} />
                  <FloatingInput label="Final Fees" type="number" value={editFormData.final_fees || ""} onChange={e => setEditFormData({
                ...editFormData,
                final_fees: parseFloat(e.target.value) || null
              })} />
                  <DatePicker label="Final Fees Received On" value={editFormData.final_fees_received_on || ""} onChange={date => setEditFormData({
                ...editFormData,
                final_fees_received_on: date
              })} showLabelOutside={false} />
                  <DatePicker label="Judgement Date" value={editFormData.judgement_date || ""} onChange={date => setEditFormData({
                ...editFormData,
                judgement_date: date
              })} showLabelOutside={false} />
                </div>
              ...
              <div className="bg-slate-50 p-4 rounded-lg">
                ...
                <div className="space-y-4">
                  <FloatingInput label="Present Status" value={editFormData.present_status || ""} onChange={e => setEditFormData({
                ...editFormData,
                present_status: e.target.value
              })} />
                  <FloatingInput label="Details" value={editFormData.details || ""} onChange={e => setEditFormData({
                ...editFormData,
      ...
      <AlertDialog open={!!deletingEntryId} onOpenChange={() => setDeletingEntryId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deletingEntryId && handleDeleteHistoryEntry(deletingEntryId)} className="bg-red-600 hover:bg-red-700">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
