...
import { Eye, RefreshCw, Building2, User, IndianRupee, History, Calendar, Menu, LogOut, LayoutDashboard, Scale, FileText, Download, Search, Filter, X, ChevronDown } from "lucide-react";
...
const LitigationAccessApplications = () => {
  ...
  const [searchCourtDistrict, setSearchCourtDistrict] = useState("");
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    // Advanced filters
    const matchesBank =
      searchBank === "" || (app.bank_name && app.bank_name.toLowerCase().includes(searchBank.toLowerCase()));

    const matchesCourtName =
      searchCourtName === "" || (app.court_name && app.court_name.toLowerCase().includes(searchCourtName.toLowerCase()));

    const matchesCourtDistrict =
      searchCourtDistrict === "" || (app.court_district && app.court_district.toLowerCase().includes(searchCourtDistrict.toLowerCase()));

    const matchesMinAmount =
      minAmount === "" || (app.loan_amount && app.loan_amount >= parseFloat(minAmount));

    const matchesMaxAmount =
      maxAmount === "" || (app.loan_amount && app.loan_amount <= parseFloat(maxAmount));

    const matchesDateFrom =
      dateFrom === "" || new Date(app.submission_date) >= new Date(dateFrom);

    const matchesDateTo =
      dateTo === "" || new Date(app.submission_date) <= new Date(dateTo);

    return matchesName && matchesStatus && matchesLoanType && matchesBank && matchesCourtName && matchesCourtDistrict && matchesMinAmount && matchesMaxAmount && matchesDateFrom && matchesDateTo;
  });
          ...
          <div className="p-4 md:p-8">
            ...
            {!loading && applications.length > 0 && (
              <div className="mb-6 bg-card rounded-lg border p-4">
                ...
                <Collapsible open={showAdvancedSearch} onOpenChange={setShowAdvancedSearch}>
                  <CollapsibleTrigger asChild>
                    <Button variant="ghost" size="sm" className="mt-4 w-full justify-start">
                      ...
                      Advanced Search
                      <ChevronDown className={`h-4 w-4 ml-auto transition-transform ${showAdvancedSearch ? "rotate-180" : ""}`} />
                    </Button>
                  </CollapsibleTrigger>
                  <CollapsibleContent className="mt-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-muted/50 rounded-lg">
                      ...
                      <div className="space-y-2">
                        <label className="text-sm font-medium">Court District</label>
                        <div className="relative">
                          <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                          <Input
                            placeholder="Search by district..."
                            value={searchCourtDistrict}
                            onChange={(e) => setSearchCourtDistrict(e.target.value)}
                            className="pl-10"
                          />
                        </div>
                      </div>
                ...
                {(searchName || (filterStatus && filterStatus !== "all") || searchLoanType || searchBank || searchCourtName || searchCourtDistrict || minAmount || maxAmount || dateFrom || dateTo) && (
