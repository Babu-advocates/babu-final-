...
import { Search, Calendar as CalendarIcon, Clock, CheckCircle, XCircle, UserCheck, Download, Eye, MapPin, Trash2, PauseCircle, Edit } from "lucide-react";
...
const Attendance = () => {
  ...
  const [editingRecord, setEditingRecord] = useState<{ username: string; date: string; currentStatus: string } | null>(null);
  ...
  const handleExportToExcel = () => {
    try {
      ...
      // Count excluded dates (paused attendance) in the selected range
      const rangeStartStr = format(monthStart, 'yyyy-MM-dd');
      const rangeEndStr = format(monthEnd, 'yyyy-MM-dd');
      const pausedDaysInMonth = excludedDates.filter(date => 
        date >= rangeStartStr && date <= rangeEndStr
      ).length;
      ...
      const exportData = filteredEmployees.map(emp => ({
        ...
        'Paused Attendance Days': pausedDaysInMonth,
  ...
  const handleEditAttendance = (username: string, date: string, currentStatus: string) => {
    setEditingRecord({ username, date, currentStatus });
  ...
  const handleUpdateAttendance = async () => {
    ...
    try {
      ...
      const { data: employeeData, error: empError } = await supabase
        ...
        .select('id, username')
        .eq('username', editingRecord.username)
        .maybeSingle();
      ...
      console.log('Deleting records for:', editingRecord.username, editingRecord.date);
      
      // First, fetch existing records to verify
      const { data: existingRecords } = await supabase
        .from('attendance_records')
        .select('*')
        .eq('employee_username', editingRecord.username)
        .eq('date', editingRecord.date);
      
      console.log('Existing records before delete:', existingRecords);

      // Delete ALL existing records for this date and employee (both check-in and check-out)
      ...
      if (newStatus === 'Present') {
        ...
        const { error: insertError } = await supabase.from('attendance_records').insert([
          {
            ...
            employee_username: editingRecord.username,
            date: editingRecord.date,
            type: 'check-in',
            timestamp: checkInTime.toISOString(),
            photo: 'admin-manual-entry',
            location: null
          },
          {
            employee_id: employeeData.id,
        ...
        console.log('Inserted Incomplete record');
      } else {
        console.log('Set as Absent - no records created');
      }
      // For 'Absent', we don't create any records (already deleted above)

      showToast.success(`Attendance marked as ${newStatus}`);
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Update attendance status for {editingRecord?.username} on {editingRecord?.date ? format(new Date(editingRecord.date), 'MMM dd, yyyy') : ''}
            </DialogDescription>
          </DialogHeader>
