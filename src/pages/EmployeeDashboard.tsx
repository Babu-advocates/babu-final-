...
import { Calendar, Building2, FileText, Clock, TrendingUp, CheckCircle, Camera, LogOut, Bell } from "lucide-react";
export default function EmployeeDashboard() {
  ...
  const fetchUnreadQueriesCount = async () => {
    try {
      const currentEmployee = localStorage.getItem('employeeUsername') || 'Nazar';

      // Get all applications assigned to this employee
      const { data: employeeApplications, error: appsError } = await supabase
        .from('applications')
        .select('application_id')
        .eq('assigned_to_username', currentEmployee);

      if (appsError) {
        console.error('Error fetching employee applications:', appsError);
        return;
      }

      if (!employeeApplications || employeeApplications.length === 0) {
        setUnreadQueriesCount(0);
        return;
      }

      const applicationIds = employeeApplications.map(app => app.application_id);

      // Count unread queries from banks for these applications
      const { count, error: queriesError } = await supabase
        .from('queries')
      ...
      if (queriesError) {
        console.error('Error fetching unread queries:', queriesError);
        return;
      }

      setUnreadQueriesCount(count || 0);
    } catch (error) {
      console.error('Error fetching unread queries count:', error);
    }
  };
  ...
  return <SidebarProvider>
      <div className="min-h-screen flex w-full bg-gradient-employee-light font-kontora">
        ...
        <div className="flex-1 flex flex-col">
          {/* Header */}
          <header className="sticky top-0 z-50 min-h-16 border-b border-border bg-card/50 backdrop-blur-sm flex items-center px-3 md:px-6 gap-2 md:gap-4 flex-wrap md:flex-nowrap py-3 md:py-0">
            ...
            <div className="flex items-center gap-2 md:gap-3 w-full sm:w-auto mt-4 lg:mt-0">
              {/* Query Notifications Button */}
              <Button
                onClick={() => navigate('/employee/applications')}
                variant="outline"
                size="icon"
                className="relative h-12 w-12 md:h-10 md:w-10 border-employee-legal/20 hover:bg-employee-legal/10"
              >
                <Bell className="h-5 w-5 md:h-4 md:w-4 text-employee-legal" />
                {unreadQueriesCount > 0 && (
                  <span className="absolute -top-1 -right-1 h-5 w-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center">
                    {unreadQueriesCount > 9 ? '9+' : unreadQueriesCount}
                  </span>
                )}
              </Button>
