...
export function QueryForm({ applicationId, currentUserType, currentUserName, currentUserEmail }: QueryFormProps) {
  ...
  // Mark incoming messages as read when the thread is visible (WhatsApp-like)
  ...
  // Whenever messages load/change, mark unread messages from the other party as read
  useEffect(() => {
    if (loading) return;
    const hasUnreadFromOtherSide = queries.some(
      (q) => q.sender_type !== currentUserType && !q.is_read
    );
    if (hasUnreadFromOtherSide) {
      markReceivedMessagesAsRead();
    }
  }, [queries, currentUserType, applicationId, loading]);
                    ...
                    <div className={`flex ${query.sender_type === currentUserType ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] rounded-lg p-3 ${
                        ...
                        <div className="flex items-center justify-between gap-2 mt-2">
                          ...
                          {/* Show double tick for sender's own messages */}
                          {query.sender_type === currentUserType && (
                            <div className="flex items-center gap-0.5">
                              <svg 
                                ...
                                viewBox="0 0 16 16" 
                                fill="none" 
                                className={query.is_read ? "text-blue-400" : "text-gray-300"}
                              >
                                <path 
                                  d="M2 8.5L5 11.5L9.5 7" 
                                  stroke="currentColor" 
                                  strokeWidth="1.5" 
                                  strokeLinecap="round" 
